...
func TestRectClosed(t *testing.T) {
	expect(t, R(10, 11, 12, 13).Closed())
}

func TestRectRawPoints(t *testing.T) {
	r := R(10, 11, 12, 13)
	points := r.RawPoints()
	expect(t, len(points) == r.NumPoints())
	for i := 0; i < len(points); i++ {
		expect(t, points[i] == r.PointAt(i))
	}
}
